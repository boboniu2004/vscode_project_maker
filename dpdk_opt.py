...
if __name__ == "__main__":
    ...
    app_list = [["/usr/bin/more", "/usr/bin", ""], ["/usr/bin/tail", "/usr/bin", ""]]
    #地址随机化开启标志：1表示开启、0表示关闭
    ASLR_flg = "1"
    #需要开启的巨页的尺寸，有2048kb和1048576kb两种
    page_size = 2048
    #需要开启的巨页的数量，必须是2的幂次方。page_size*page_cnt*1024必须小于系统总内存大小
    page_cnt = 256
    ...
    if "install"==sys.argv[1]:
        error = dpdk_scrits.Install_dpdkenv(sys.argv[0], cpu_list, page_size)
    elif "uninstall"==sys.argv[1]:
        error  = dpdk_scrits.Uinstall_dpdkenv(sys.argv[0], app_list)
    elif "initenv"==sys.argv[1]:
        error  = dpdk_scrits.Set_dpdkenv(ASLR_flg, page_size, page_cnt, devbind_path, \
